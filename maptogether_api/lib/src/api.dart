...
String stringify(LeaderboardType type) {
    switch(type) {
        ...
        case LeaderboardType.montly: return "montly";
        case LeaderboardType.all_time: return "all_time";
    }
    throw 'Missing case';
}
...
class InvalidRegionException implements Exception {
	...
	InvalidRegionException([this._message = 'Region cannot be global or leaderboard']);
...
class MapTogetherApi {
  ...
  Future<String> leaderboard(String base, LeaderboardType type) =>
  	_get('leaderboard/$base/${stringify(type)}').then(_checkRequest('getting leaderboard'));
